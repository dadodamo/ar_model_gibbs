...
//    std::vector<Eigen::VectorXd> o_store(this->T+1);
    ...
    std::vector<Eigen::VectorXd> o_store = this->ot; //debug

    Eigen::VectorXd mu_0 = this->mu_0_true; // debug
    //Eigen::VectorXd mu_0(this->N);

    //double rho;
    double rho = this->rho_true; //debug
...
//        Eigen::EigenMultivariateNormal<double> o_zero_sampler(o_zero_update_mean, o_zero_update_cov, use_cholesky, seed = i);
//        o_store[0] = o_zero_sampler.samples(1);
//
//        // write in proto file
//
...
//            Eigen::EigenMultivariateNormal<double> ot_sampler(ot_update_mean, ot_update_cov, use_cholesky, seed = i);
...
//                                                               beta, sigma_eps, rho);
//        Eigen::EigenMultivariateNormal<double> ot_sampler(oT_update_mean, oT_update_cov, use_cholesky, seed = i);
//        o_store[this->T] = ot_sampler.samples(1);
//
//
//        o_data::vector* o_T_vector = o_matrix->add_vec(); //maybe not neccessary
//        o_T_vector->mutable_vec_value()->Add(o_store[T].begin(), o_store[T].end());
...
//
        ...
        Eigen::EigenMultivariateNormal<double> beta_sampler(beta_update_mean, beta_update_cov,use_cholesky = false, seed = i);
        beta = beta_sampler.samples(1);
