...
void ar_model::sample() const {
  ...
  //  std::vector<Eigen::VectorXd> o_store = ot; //debug
    ...
    Eigen::VectorXd beta_prior = Eigen::VectorXd::Zero(p);
    Eigen::VectorXd o_prior = Eigen::VectorXd::Zero(N);

    Eigen::EigenMultivariateNormal<double> beta_sampler(beta_prior, beta_sig_prior*id_p, use_cholesky, seed);
    Eigen::EigenMultivariateNormal<double> o_sampler(o_prior, id_N, use_cholesky, seed);
