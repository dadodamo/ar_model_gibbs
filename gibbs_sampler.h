#ifndef AR_GIBBS_GIBBS_SAMPLER_H
#define AR_GIBBS_GIBBS_SAMPLER_H

#include<iostream>
#include<cmath>
#include "dist_sample.cpp"
#include "Eigen/dense"
...
// beta
    Eigen::VectorXf calc_mean_beta(std::vector<Eigen::MatrixXf>& x_store_vec, Eigen::MatrixXf& covar_w_inv, std::vector<Eigen::VectorXf>& ot_store_vec, float& rho){
    ...
    Eigen::MatrixXf calc_cov_beta(std::vector<Eigen::MatrixXf>& x_store_vec, Eigen::MatrixXf& covar_w_inv, float& sigma_beta_prior){
        ...
        for (int t = 0; t < x_store_vec.size()-1; ++t) {
            temp += x_store_vec[t].transpose() * covar_w_inv * x_store_vec[t] + id_mat * sigma_beta_prior;
        }
        return temp.inverse();
    };

// rho
    float calc_mean_rho(std::vector<Eigen::MatrixXf>& x_store_vec, Eigen::MatrixXf& covar_w_inv, std::vector<Eigen::VectorXf>& ot_store_vec, Eigen::VectorXf& beta){
        float mean = 0;
        for (int t = 1; t < x_store_vec.size(); ++t) {
    ...
    float calc_var_rho(std::vector<Eigen::VectorXf>& ot_store_vec, Eigen::MatrixXf& covar_w_inv, float& sigma_rho_prior){
        float var = 0;
...
// O_T
    Eigen::VectorXf calc_mean_eff_T(Eigen::VectorXf& y_T, Eigen::MatrixXf& x_T, Eigen::MatrixXf& covar_w_inv, Eigen::VectorXf& o_prev,
...
// O_0
    Eigen::VectorXf calc_mean_eff_0(Eigen::MatrixXf&  x_1, Eigen::MatrixXf& covar_w_inv, Eigen::VectorXf& o_1, Eigen::VectorXf& beta, Eigen::MatrixXf& S_0_inv,
    ...
    Eigen::MatrixXf calc_cov_eff_0(Eigen::MatrixXf& covar_w_inv, Eigen::MatrixXf& S_0_inv, float& rho, float& sigma_0){
        Eigen::MatrixXf cov(covar_w_inv.cols(), covar_w_inv.cols());
...
// O_t
    Eigen::VectorXf calc_mean_eff_t(Eigen::VectorXf& y_t, Eigen::MatrixXf& x_curr, Eigen::MatrixXf x_next, Eigen::MatrixXf& covar_w_inv, Eigen::VectorXf& o_prev, Eigen::VectorXf& o_next,
        ...
        mean = y_t * 1/sigma_eps + covar_w_inv * (x_curr*beta + rho  * (o_prev + o_next - x_next*beta));
        return mean;
    };
...
#endif //AR_GIBBS_GIBBS_SAMPLER_H
